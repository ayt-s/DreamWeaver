...
class NovelAnalysis(BaseModel):
    ...
    animal_characters: list[str] = Field(
        default_factory=list,
        description=(
            "characters 里**属于动物/灵兽/宠物/妖**的角色名，必须与 characters 的键完全一致。"
            "人形角色一律不要放。特别注意：绰号或名字里带动物字的**人类**"
            "（例如被叫「黑牛」的壮汉、「二狗」的村民）不算动物，不要放进来。"
            "没有非人角色就填空数组。"
        ),
    )
...
- animal_characters 列出 characters 中**属于动物/灵兽/宠物/妖**的角色名，必须与 characters 的键一致；
  只列非人角色 —— 名字或绰号里带动物字的**人类**（如叫「黑牛」的壮汉）**不算**，没有非人角色就填空数组 []。
