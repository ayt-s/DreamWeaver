...
def _qc_route(state: CreativeSessionState) -> str:
    ...
    **画布模式（segments 非空）→ synthesizer，只报告不重生。**
    ...
    2026-09-18 判据标定完成后**仍保留**这个口径，但理由换了 —— 不再是「阈值没标定」，
    而是画布模式的段重生有**独立的人工入口**（画廊/画布的段重生），
    自动重生会盖掉用户手工挑过的段落。

    **标准模式**：`AGENT_QC_AUTOFIX=0`（代码默认）时同样只报告。
    判据已收窄为三条确定性项（下载残片 / 黑帧过半 / 空帧，blur 退出 passed），
    44 个唯一真实段回放误报 0/44 —— 本机通过 .env 设成 1，显式打开才走自愈循环。
    """
    if state.get("segments"):
