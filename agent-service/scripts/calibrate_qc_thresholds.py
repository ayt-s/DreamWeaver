"""QC 阈值离线标定：用真实产物定黑帧/模糊阈值的分位数依据。

## 为什么不能跳过（Task A6）

`app/tools/qc.py` 里的两个阈值是拍脑袋定的：
- `BLACK_RATIO_THRESHOLD = 0.95`（全黑像素比例）
- `BLUR_VARIANCE_THRESHOLD = 50.0`（Laplacian 方差）
- 以及 `passed = black_ratio <= 阈值 and blur_ratio <= 0.5` 里那个写死的 0.5
